...
const auth = {
  checkAPIKey: async function (req, res, next) {
    // Need to go through a check first for those request that don't need an API Key.
    if (req.path == "/") {
      // Documentation
      return next();
    }
    ...
    if (req.path == "/auth/logout/google") {
      // Google
      return next();
    }

    if (req.path == "/auth/login/google/error") {
      // Google
      return next();
    }

    auth.validAPIKey(
      req.query.api_key || req.body.api_key,
      next,
      req.path,
      res
    );
  ...
  checkValidAdmin: async function (req, res, next) {
    if (
      req.body.api_key === process.env.REACT_APP_ADMIN_API_KEY ||
      req.query.api_key === process.env.REACT_APP_ADMIN_API_KEY
    ) {
  ...
  validTokenKey: async function (req, res, next) {
    // Look if token is valid
    if (req.headers["x-access-token"] !== undefined) {
      if (
        ...
        req.body.api_key == process.env.REACT_APP_USER_MOBILE_API_KEY ||
        req.query.api_key == process.env.REACT_APP_USER_MOBILE_API_KEY
      ) {
        return auth.userCheckToken(req, res, next);
      } else if (
        req.body.api_key == process.env.REACT_APP_USER_WEBB_API_KEY ||
        req.query.api_key == process.env.REACT_APP_USER_WEBB_API_KEY
      ) {
        return auth.userCheckToken(req, res, next);
      }
    } else if (req.user) {
  ...
  adminLogin: async function (res, body, path) {
    // Admin login
  ...
  adminRegister: async function (res, body, path) {
    ...
    if (!adminEmail || !adminPassword) {
      ...
      // If adminFirstName or adminLastName is undefined
      return res.status(401).json({
        errors: {
          status: 401,
          source: "POST /auth" + path,
  ...
  adminCheckToken: function (req, res, next) {
    // Check the x-access-token from admin
    let token = req.headers["x-access-token"];

    if (!token) {
      return res.status(401).json({
        // If no token in request headers response with error code 401
        errors: {
          status: 401,
          source: req.path,
    ...
    jwt.verify(token, jwtSecret, function (err, decoded) {
      ...
      if (err) {
        return res.status(500).json({
          // If error response with error code 500
          errors: {
            status: 500,
            source: req.path,
  ...
  userLogin: async function (res, body, path) {
    // Admin login
  ...
  userRegister: async function (res, body, path) {
    // Register a new Admin
    const userEmail = sanitize(body.email);
    const userPassword = sanitize(body.password);
    ...
    if (!userEmail || !userPassword) {
      ...
      // If userFirstName or userLastName is undefined
      return res.status(401).json({
        errors: {
          status: 401,
          source: "POST auth" + path,
  ...
  userCheckToken: function (req, res, next) {
    // Check the x-access-token from user
    let token = req.headers["x-access-token"];
