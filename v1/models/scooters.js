...
const scooters = {
    ...
    if (!statusList.includes(scooterStatus)) {
      return res.status(400).json({
        errors: {
          status: 400,
          detail:
            "Required attribute status must contain one of these statuses: " +
            statusList,
        },
      });
    }
  ...
  getAllScootersOverview: async function (res, path) {
    ...
    try {
      let db = client.db("spark-rentals");
      let scooters_collection = db.collection("scooters");
      scootersOverview = await scooters_collection
        .find({})
        .project({
          _id: 1,
          name: 1,
  ...
  editScooter: async function (res, body, path) {
    ...
    try {
      let db = client.db("spark-rentals");
      let scooters_collection = db.collection("scooters");
      let scooter = await scooters_collection.findOne({
      ...
      if (scooter === null) {
        return res.status(401).json({
          errors: {
            ...
            title: "Scooter not exists in database",
            detail:
              "The scooter dosen't exists in database with the specified scooterID.",
          },
        });
      }
  ...
  getIsLockedField: async function () {
    //
  },
  ...
  editCoordinatesScooter: async function (res, body, path) {
    ...
    if (isNaN(scooterLongitude) || isNaN(scooterLatitude)) {
      return res.status(400).json({
        errors: {
          status: 400,
          detail:
            "Required attributes longitude and latitude must be valid numbers.",
        },
      });
    }
    ...
    try {
      ...
      if (scooter === null) {
        return res.status(401).json({
          errors: {
            ...
            title: "Scooter not exists in database",
            detail:
              "The scooter doesn't exist in the database with the specified scooter_id.",
          },
        });
      }
      ...
      await scooters_collection.updateOne(
        {_id: ObjectId(scooterId)},
        {$set: {coordinates: coordinatesField}}
      );
    } catch (e) {
      return res.status(500).send();
    } finally {
      ...
      if (user === null) {
        return res.status(401).json({
          errors: {
            ...
            detail:
              "The user dosen't exists in database with the specified user_id.",
          },
        });
      }
      ...
      if (user.balance < 50) {
        return res.status(402).json({
          errors: {
            ...
            detail:
              "The user does not have enough balance in their account. At least SEK 50 is required. The user balance is: " +
              user.balance +
              " SEK.",
          },
        });
      ...
      if (scooter.status !== "Available") {
        return res.status(401).json({
          errors: {
            ...
            detail:
              "The specified scooter is not available to rent. The status of the scooter is: " +
              scooter.status +
              ".",
          },
        });
  ...
  calculateRidePrice: async function (res, scooter, endTime) {
    ...
    let arrayStartPosition = [
      scooter.trip.startPosition.longitude,
      scooter.trip.startPosition.latitude,
    ];
    ...
    let startTimeSeconds =
      +startTimeSplit[0] * 60 * 60 +
      +startTimeSplit[1] * 60 +
      +startTimeSplit[2];
    let endTimeSeconds =
      +endTimeSplit[0] * 60 * 60 + +endTimeSplit[1] * 60 + +endTimeSplit[2];
    ...
    if (city === null || !Object.keys(city).length) {
      return res.status(401).json({
        errors: {
          ...
          detail:
            "The City dosen't exists in database with the specified scooter.owner.",
        },
      });
    }
    ...
    // Get the parkinzones
    let noParkingZones = city.zones.filter((x) =>
      x.zoneType.includes("noParkingZone")
    );
    let parkingZones = city.zones.filter((x) =>
      x.zoneType.includes("parkingZone")
    );
    let bonusParkingZones = city.zones.filter((x) =>
      x.zoneType.includes("bonusParkingZone")
    );
    let chargingZones = city.zones.filter((x) =>
      x.zoneType.includes("chargingZone")
    );
    ...
    for (noParkingZone in noParkingZones) {
      if (
        classifyPoint(
          noParkingZones[noParkingZone].coordinates,
          arrayStartPosition
        ) == -1
      ) {
        // If user started the scooter trip on a no parking zone
      ...
      if (
        ...
        // If user ended the scooter trip on a no parking zone
    ...
    for (bonusParkingZone in bonusParkingZones) {
      if (
        classifyPoint(
          bonusParkingZones[bonusParkingZone].coordinates,
          arrayStartPosition
        ) == -1
      ) {
        // If user started the scooter trip on a bonus parking zone
      ...
      if (
        ...
        // If user ended the scooter trip on a bonus parking zone
    ...
    for (chargingZone in chargingZones) {
      if (
        classifyPoint(
          chargingZones[chargingZone].coordinates,
          arrayStartPosition
        ) == -1
      ) {
        // If user started the scooter trip on a charging zone
      ...
      if (
        ...
        // If user ended the scooter trip on a charging zone
    ...
    for (parkingZone in parkingZones) {
      if (
        classifyPoint(
          parkingZones[parkingZone].coordinates,
          arrayStartPosition
        ) == -1
      ) {
        // If user started the scooter trip on a parking zone
      ...
      if (
        ...
        // If user ended the scooter trip on a parking zone
    ...
    if (
      (startInNoParkinZone && endInParkinZone && !endInNoParkinZone) ||
  ...
  stopScooter: async function (res, scooter_id, user_id, path) {
    ...
    try {
      let db = client.db("spark-rentals");
      let scooters_collection = db.collection("scooters");
      var scooter = await scooters_collection.findOne({
        _id: ObjectId(scooterId),
      });
      ...
      await scooters_collection.updateOne(
        {_id: ObjectId(scooterId)},
        {$set: {status: "Available", "trip.endTime": endTime}}
      );
    } catch (e) {
      return res.status(500).send();
    } finally {
    ...
    try {
      ...
      let arrayCalculate = await scooters.calculateRidePrice(
        res,
        scooter,
        endTime
      ); // Get price and time
